...
`Back-end` `Authentication` `Python`
...
- validate the switch by using the environment variables
...
Update `SessionAuth` class:
- Create an instance method `def user_id_for_session_id(self, session_id: str = None) -> str:` that returns `user_id` based on `session_id`:
    - Return None if `session_id` is `None`
    - Return None if `session_id` is not a string
    - Return None if `session_id` is not a key of the dictionary `user_id_by_session_id`
